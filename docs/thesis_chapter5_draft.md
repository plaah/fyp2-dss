...
#### 5.2.2.1 Feature Ablation and Robustness Analysis

To assess whether the high predictive accuracy of the deployed model was attributable to genuine learning or to inadvertent data leakage, a feature ablation experiment was conducted by retraining XGBoost on a reduced 27-feature set from which the `final_success` column was removed. The `final_success` feature is a derived binary indicator computed as the logical conjunction of `idrg_grouping_success` and `inacbg_grouping_success`, and its strong SHAP dominance — impact 5.10 versus 0.03 for the next most influential feature — raised the question of whether the model was trivially reading a proxy of the target label. The no-leakage model was trained using an identical hyperparameter configuration (early stopping on a 15% validation split, then rebuilt at the identified best iteration of 80 on the full training set) and evaluated on the same held-out test partition. The experiment yielded an accuracy of 98.79%, a weighted F1-score of 0.9879, and an AUC-ROC of 0.9984 — a reduction of 1.06 percentage points in accuracy, 0.0106 in F1, and 0.0012 in AUC-ROC relative to the 28-feature baseline. The no-leakage model is saved as `models/xgboost_no_leakage.pkl`.

The modest magnitude of the performance drop confirms two findings of clinical relevance. First, `final_success` does not constitute true data leakage in the deployment context: both of its constituent inputs — `idrg_grouping_success` and `inacbg_grouping_success` — are observable system flags that are submitted as runtime inputs to the prediction API and are therefore legitimately available at inference time. The feature encodes real operational state rather than an artifact of label construction. Second, and more importantly, the 98.79% accuracy achieved on 27 features demonstrates that the model has learned genuine ICD coding patterns from the remaining feature set — including the ICD-10 code frequency encodings, the tariff ratio, the claim stage progression, and the iDRG/INACBG validity flags — rather than relying exclusively on a single dominant shortcut. For the production system, the 28-feature model (`models/best_model.pkl`) is recommended, as all input features are available at the point of clinical prediction. The 27-feature no-leakage variant is retained for deployment scenarios in which a prediction must be made before either grouper has been executed, such as a pre-submission risk screening tool integrated upstream of the iDRG workflow.
